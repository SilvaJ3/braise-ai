...
describe('aliasDepuisNom', () => {
  it('nettoie accents, espaces et ponctuation', () => {
    expect(aliasDepuisNom('Au Coin du Feu')).toBe('aucoindufeu')
    expect(aliasDepuisNom("L'Atelier Bougie & Cie")).toBe('latelierbougiecie')
    expect(aliasDepuisNom('Épicerie Fine')).toBe('epiceriefine')
  })
  it('ne rend jamais une chaîne vide', () => {
    expect(aliasDepuisNom('')).toBe('bons')
    expect(aliasDepuisNom('!!!')).toBe('bons')
  })
  it('borne la longueur', () => {
    expect(aliasDepuisNom('a'.repeat(80))).toHaveLength(40)
  })
})

describe('adresseExpediteur', () => {
  it('compose nom + adresse de l’app', () => {
    expect(adresseExpediteur('Au Coin du Feu', 'aucoindufeu', 'braise.io')).toBe(
      'Au Coin du Feu <aucoindufeu@braise.io>',
    )
  })
  it('tombe sur l’adresse seule si le nom est vide', () => {
    expect(adresseExpediteur('  ', 'bons', 'braise.io')).toBe('bons@braise.io')
  })
  it('neutralise une injection d’en-tête', () => {
    const a = adresseExpediteur('X\r\nBcc: pirate@example.com', 'bons', 'braise.io')
...
describe('envoyerMail', () => {
  it('poste au bon endroit, avec en-têtes et pièce jointe', async () => {
    ...
    expect(body.from).toBe('Au Coin du Feu <aucoindufeu@braise.io>')
